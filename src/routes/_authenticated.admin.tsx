  ...
  if (isLoading && !data) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Activity className="w-8 h-8 animate-pulse text-neutral-400" />
      </div>
    );
  }
  ...
  if (isError && !data) {
    return (
      <div className="p-8 text-center">
        ...
        <h2 className="text-xl font-semibold text-neutral-900">System Unavailable</h2>
        <p className="text-neutral-600 mt-2">Failed to fetch system health</p>
      </div>
    );
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center border-b pb-2">
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${data?.task_manager.status === "healthy" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
              >
